...
<div class="d-flex align-items-center justify-content-center" style="height: 100vh; ">
    <div class="container-fluid col-md-8 bg-white  shadow-sm  rounded ">
        <h3 class="mt-3 mb-3">Solicitantes</h3>
        ...
        <form action="/costumer" method="get">
            <button class="btn btn-success mb-3 " type="submit">Novo Solicitante</button>
        </form>
    <c:if test="${empty costumers}">
        <h5>Ainda não há solicitantes cadastrados.</h5>
    </c:if>
    <c:if test="${not empty costumers}">
        ...
        <div class="overflow-auto" style="max-height: 500px; height: 500px;">
            <table class="table table-striped table-hover border border-light rounded">
                ...
                <tbody>
                <c:forEach var="u" items="${costumers}">
                    <tr>
                        <td>${u.id}</td>
                        <td>${u.nome}</td>
                        <td>${u.cpf}</td>
                        ...
                        <td>${u.idade}</td>
                        <td>${u.score}</td>
                        <td>${u.limiteDisponivel}</td>
                        <td>
                            <a class="btn btn-primary" href="/costumer/update/${u.id}" data-id="${u.id}"
                               data-id-nome="${u.nome}">
                            ...
                            <button type="button"
                                    ...
                                    data-id="${u.id}" data-id-nome="${u.nome}" data-bs-toggle="modal"
...
<script>
    ...
    function updateDeleteLink(id) {
        link.attr('href', '/costumer/delete/' + id);
    }
