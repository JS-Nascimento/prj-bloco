...
<div class="d-flex align-items-center justify-content-center" style="height: 100vh; ">
    <div class="container-fluid col-md-8 bg-white  shadow-sm  rounded ">
        ...
        <div class="overflow-auto" style="max-height: 500px; height: 500px;">
            <table class="table align-middle table-striped table-hover border border-light rounded">
                <thead>
                <tr>
                    <th>Id</th>
                    <th>Nível</th>
                ...
                <c:forEach var="u" items="${users}">
                    <tr>
                        <td>${u.id}</td>
                        <c:choose>
                            <c:when test="${u.nivelAcesso == 1}">
                                <td><span class="badge rounded-pill bg-primary">Administrador</span></td>
                            </c:when>
                            <c:when test="${u.nivelAcesso == 2}">
                                <td><span class="badge rounded-pill bg-warning text-dark">Padrão</span></td>
                            </c:when>
                            <c:when test="${u.nivelAcesso == 3}">
                                <td><span class="badge rounded-pill bg-success">Diretoria</span></td>
                            </c:when>
                        </c:choose>
                        <td>${u.nome}</td>
                        <td>${u.email}</td>
                        ...
                        <td>
                            <a class="btn btn-primary ${userLogged.nivelAcesso == 2 || u.nivelAcesso == 3 && userLogged.id == u.id ? 'disabled' : ''}"
                               href="/user/update/${u.id}" data-id="${u.id}" data-id-nome="${u.nome}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                            ...
                            <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                    data-bs-target="#staticBackdrop" data-id="${u.id}"
                                    data-id-nome="${u.nome}" ${userLogged.nivelAcesso == 3 || userLogged.nivelAcesso == 2 || userLogged.id == u.id ? 'disabled' : ''}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
